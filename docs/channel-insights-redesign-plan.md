...
- [ ] **Podfaza 0.7 — UI moduli:** DOM, dialogs, dashboards in video list so ekstrahirani z nespremenjenim DOM contractom; avtomatski init/render in ciljni dinamični DOM testi so zeleni. Checkbox ostaja odprt do polnega ročnega browser smoke testa v Podfazi 0.9.
- [ ] **Podfaza 0.8 — bootstrap:** `app.js` je minimalen orkestrator brez krožnih odvisnosti in podvojenih helperjev.
